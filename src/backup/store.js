...
const getters = {
    ...
    selectedMedia: state => {
        return state.selectedMedia[state.pickerId];
    },

    selectedMediaIds: state => {
        return state.selectedMedia[state.pickerId].map(({ id }) => id);
    },
...
const mutations = {
    ...
    setSelectedMedia(state, { id, media }) {
        console.log(id, media);
        state.selectedMedia[id] = media;
    },
...
const actions = {
    open({ commit, dispatch }, { pickerId, limit, selectedMediaIds, acceptedExtensions }) {
        commit('setLimit', limit || null);
    ...
    setSelectedMedia({ commit, getters }, { pickerId, mediaIds }) {
        commit('setSelectedMedia', {
            id: pickerId,
            media: (Array.isArray(mediaIds) && mediaIds.length)
                ? getters.activeMedia(mediaIds)
                : []
        });
    },

    setActiveMedia({ commit }, media) {
        commit('setActiveMedia', media);
    },
